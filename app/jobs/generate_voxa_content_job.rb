class GenerateVoxaContentJob < ApplicationJob
  ...
  def perform(strategy_plan_id)
    Rails.logger.info "GenerateVoxaContentJob: Starting job for strategy plan #{strategy_plan_id}"

    strategy_plan = CreasStrategyPlan.find(strategy_plan_id)
    Rails.logger.info "GenerateVoxaContentJob: Found strategy plan #{strategy_plan.id} (user: #{strategy_plan.user_id}, brand: #{strategy_plan.brand_id})"

    strategy_plan.update!(status: :processing)
    Rails.logger.info "GenerateVoxaContentJob: Strategy plan status updated to processing"
    ...
    begin
      ...
      Rails.logger.info "GenerateVoxaContentJob: Expected content count: #{expected_count}"

      # Generate Voxa refinements
      Rails.logger.info "GenerateVoxaContentJob: Generating strategy plan data and brand context"
      ...
      Rails.logger.info "GenerateVoxaContentJob: Building prompts for OpenAI"
      ...
      # Call OpenAI
      Rails.logger.info "GenerateVoxaContentJob: Calling OpenAI API (model: gpt-4o-mini, temperature: 0.5)"
      ...
      Rails.logger.info "GenerateVoxaContentJob: OpenAI response received (#{response&.length || 0} characters)"
      ...
      Rails.logger.info "GenerateVoxaContentJob: Parsing OpenAI response"
      parsed_response = JSON.parse(response)
      voxa_items = parsed_response.fetch("items")
      Rails.logger.info "GenerateVoxaContentJob: Found #{voxa_items.count} items in Voxa response"

      # Process Voxa items and ensure we have the right count
      Rails.logger.info "GenerateVoxaContentJob: Processing Voxa items"
      ...
      Rails.logger.error "GenerateVoxaContentJob: JSON parsing error for strategy plan #{strategy_plan.id}: #{e.message}"
      handle_error(strategy_plan, "Voxa returned non-JSON content: #{e.message}")
    rescue KeyError => e
      Rails.logger.error "GenerateVoxaContentJob: Missing key in Voxa response for strategy plan #{strategy_plan.id}: #{e.message}"
      handle_error(strategy_plan, "Voxa response missing expected key: #{e.message}")
    rescue StandardError => e
      Rails.logger.error "GenerateVoxaContentJob: Unexpected error for strategy plan #{strategy_plan.id}: #{e.message}"
      Rails.logger.error "GenerateVoxaContentJob: Error backtrace: #{e.backtrace.join("\n")}" if e.backtrace
  ...
  def ensure_draft_content_exists(strategy_plan)
    ...
    Rails.logger.info "GenerateVoxaContentJob: Checking draft content - existing count: #{existing_count}"

    if existing_count == 0
      Rails.logger.info "GenerateVoxaContentJob: No existing content found for strategy plan #{strategy_plan.id}, creating draft content"
      ...
      Rails.logger.info "GenerateVoxaContentJob: Created #{new_count} draft content items"
    else
      Rails.logger.info "GenerateVoxaContentJob: Using existing #{existing_count} content items"
    end
  end
  ...
  def calculate_expected_content_count(strategy_plan)
    ...
    return 0 unless strategy_plan.weekly_plan.present?
  ...
  def process_voxa_items(strategy_plan, voxa_items, expected_count)
    ...
    Rails.logger.info "GenerateVoxaContentJob: Processing #{voxa_items.count} Voxa items in database transaction"
    CreasContentItem.transaction do
      voxa_items.each_with_index do |item, index|
        Rails.logger.debug "GenerateVoxaContentJob: Processing item #{index + 1}/#{voxa_items.count}: #{item['id']}"
    ...
    Rails.logger.info "GenerateVoxaContentJob: Successfully processed #{actual_count} Voxa items"

    # Validate count matches expectation
    if actual_count != expected_count
      Rails.logger.warn "GenerateVoxaContentJob: Voxa processing count mismatch for strategy plan #{strategy_plan.id}: expected #{expected_count}, got #{actual_count}"
      ...
      Rails.logger.info "GenerateVoxaContentJob: Content count matches expectation: #{actual_count}"
    end

    # Return final count
    final_count = strategy_plan.creas_content_items.count
    Rails.logger.info "GenerateVoxaContentJob: Final content items count: #{final_count}"
    final_count
  end
  ...
  def ensure_complete_content_set(strategy_plan, expected_count, actual_count)
    ...
    if actual_count < expected_count
      ...
      Rails.logger.info "GenerateVoxaContentJob: Running ContentItemInitializerService to ensure complete content set (missing #{missing_count} items)"
      Creas::ContentItemInitializerService.new(strategy_plan: strategy_plan).call

      # Log the result
      final_count = strategy_plan.creas_content_items.count
      Rails.logger.info "GenerateVoxaContentJob: After quantity guarantee, final count: #{final_count}/#{expected_count}"
    end
  end
  ...
  def handle_error(strategy_plan, error_message)
    Rails.logger.error "GenerateVoxaContentJob: Handling error for strategy plan #{strategy_plan.id}: #{error_message}"
    ...
    Rails.logger.error "GenerateVoxaContentJob: Strategy plan #{strategy_plan.id} marked as failed with error: #{error_message}"
    Rails.logger.error "GenerateVoxaContentJob: Error metadata saved to strategy plan meta field"
  end
  ...
  def normalize_template(template)
    ...
    if normalized_template
      Rails.logger.info "GenerateVoxaContentJob: Normalized template '#{template}' to '#{normalized_template}'"
      return normalized_template
    end

    # If no mapping found, log the unknown template and default to solo_avatars
    Rails.logger.warn "GenerateVoxaContentJob: Unknown template '#{template}', defaulting to 'solo_avatars'"
    "solo_avatars"
  end
end
