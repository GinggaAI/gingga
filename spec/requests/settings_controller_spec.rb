...
RSpec.describe SettingsController, type: :request do
  ...
  describe 'PATCH #update_openai' do
    context 'with valid openai api key' do
      it 'saves the API key and redirects with success message' do
        # Mock the API token validator to return success
        allow(ApiTokenValidatorService).to receive_message_chain(:new, :call).and_return({ valid: true })

        patch update_openai_settings_path(brand_slug: brand.slug, locale: :en), params: { openai_api_key: 'sk-test-key-123', mode: 'production' }
    ...
    context 'with empty api key' do
      it 'redirects with error message' do
        patch update_openai_settings_path(brand_slug: brand.slug, locale: :en), params: { openai_api_key: '', mode: 'production' }
  ...
  describe 'POST #validate_openai_api' do
    let!(:openai_token) do
    ...
    context 'when validation succeeds' do
      it 'redirects with success message' do
        ...
        post validate_openai_api_settings_path(brand_slug: brand.slug, locale: :en)
