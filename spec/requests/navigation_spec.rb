...
RSpec.describe "Basic Navigation", type: :request do
  ...
  describe "Main Application Pages" do
    context "when user is authenticated" do
      it "displays the brand edit page (/my-brand)" do
        get my_brand_path(brand_slug: brand.slug, locale: :en)
      ...
      it "displays the viral ideas page (/viral_ideas)" do
        get viral_ideas_path(brand_slug: brand.slug, locale: :en)
        expect(response).to have_http_status(:success)
      end

      it "displays the auto creation page (/auto_creation)" do
        get auto_creation_path(brand_slug: brand.slug, locale: :en)
        expect(response).to have_http_status(:success)
      end

      it "displays the analytics page (/analytics)" do
        get analytics_path(brand_slug: brand.slug, locale: :en)
        expect(response).to have_http_status(:success)
      end

      it "displays the community page (/community)" do
        get community_path(brand_slug: brand.slug, locale: :en)
        expect(response).to have_http_status(:success)
      end
  ...
  describe "Page Content and Structure" do
    ...
    it "includes proper navigation menu in all pages" do
      pages = [
        planning_path(brand_slug: brand.slug, locale: :en),
        my_brand_path(brand_slug: brand.slug, locale: :en),
        settings_path(brand_slug: brand.slug, locale: :en)
      ]
